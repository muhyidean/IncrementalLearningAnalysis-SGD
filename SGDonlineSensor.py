...
df=pandas.read_csv( 'datasets/sensor.csv') # Read data from CSV
array=df.values                                     # Convert to array

X=array[:,0:48]                                     # Seperate classifying features
Y=array[:,48]                                       # Seperate target
...
for e in exp_nums:
    ...
    for i in range(n_iter):
        ...
        for b in range(num_batches):                                            # this range is related to the number of splits
            ...
            numpy.savetxt("sensor/results/SGDResult" + str(n) + ".csv",  results, fmt='%.5f', delimiter=';', newline='\n') # export results file
            print(clf_score)
        ...
        numpy.savetxt("sensor/times/SGDTT" + str(n) + ".csv", times, fmt='%.5f')    # export file

    timesmean[0] = times.mean()
    numpy.savetxt("sensor/times/SGDTMEAN" + str(n) + ".csv", times, fmt='%.5f')  # export file
    ...
    numpy.savetxt("sensor/results/SGDAVG" + str(n) + ".csv", rmean, fmt='%.5f')
    numpy.savetxt("sensor/results/SGDSTD" + str(n) + ".csv", rstd, fmt='%.5f')
    ...
    numpy.savetxt("sensor/means/CONDRIFTRATE.csv", concept_drift_rate, fmt='%.5f')
...
numpy.savetxt("sensor/means/SGDAVGALLEXP.csv", rmeans, fmt='%.5f') # Averages
numpy.savetxt("sensor/means/SGDSTDALLEXP.csv", rstds, fmt='%.5f') # Standard Deviation
numpy.savetxt("sensor/means/SGDTIMESALLEXP.csv", timesmeans, fmt='%.5f') # Times
numpy.savetxt("sensor/means/SGDCDRALLEXP.csv", CDRmeans, fmt='%.5f') # Concept drift rate
numpy.savetxt("sensor/means/SGDPRECISION.csv", pre_means, fmt='%.5f') # Averages
numpy.savetxt("sensor/means/SGDRECALL.csv", rec_means, fmt='%.5f') # Averages
numpy.savetxt("sensor/means/SGDF1.csv", f1_means, fmt='%.5f') # Averages
