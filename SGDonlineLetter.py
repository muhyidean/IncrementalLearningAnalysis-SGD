...
df=pandas.read_csv( 'datasets/letter.csv') # Read data from CSV
array=df.values                                     # Convert to array

X=array[:,1:17]                                     # Seperate classifying features
Y=array[:,0]                                       # Seperate target
...
for e in exp_nums:
    ...
    for i in range(n_iter):
        ...
        for b in range(num_batches):                                            # this range is related to the number of splits
            ...
            numpy.savetxt("letter/results/SGDResult" + str(n) + ".csv",  results, fmt='%.5f', delimiter=';', newline='\n') # export results file
            print(clf_score)
        ...
        numpy.savetxt("letter/times/SGDTT" + str(n) + ".csv", times, fmt='%.5f')    # export file

    timesmean[0] = times.mean()
    numpy.savetxt("letter/times/SGDTMEAN" + str(n) + ".csv", times, fmt='%.5f')  # export file
    ...
    numpy.savetxt("letter/results/SGDAVG" + str(n) + ".csv", rmean, fmt='%.5f')
    numpy.savetxt("letter/results/SGDSTD" + str(n) + ".csv", rstd, fmt='%.5f')
    ...
    numpy.savetxt("letter/means/CONDRIFTRATE.csv", concept_drift_rate, fmt='%.5f')
...
numpy.savetxt("letter/means/SGDAVGALLEXP.csv", rmeans, fmt='%.5f') # Averages
numpy.savetxt("letter/means/SGDSTDALLEXP.csv", rstds, fmt='%.5f') # Standard Deviation
numpy.savetxt("letter/means/SGDTIMESALLEXP.csv", timesmeans, fmt='%.5f') # Times
numpy.savetxt("letter/means/SGDCDRALLEXP.csv", CDRmeans, fmt='%.5f') # Concept drift rate
numpy.savetxt("letter/means/SGDPRECISION.csv", pre_means, fmt='%.5f') # Averages
numpy.savetxt("letter/means/SGDRECALL.csv", rec_means, fmt='%.5f') # Averages
numpy.savetxt("letter/means/SGDF1.csv", f1_means, fmt='%.5f') # Averages
