...
df=pandas.read_csv( 'datasets/elec2.csv') # Read data from CSV
array=df.values                                     # Convert to array

X=array[:,0:7]                                     # Seperate classifying features
Y=array[:,7]                                       # Seperate target
scaler=MinMaxScaler(feature_range=(0,1))            # Decale normilizing method Min/Max
rX=scaler.fit_transform(X)                          # Normalize X array
exp_nums = [50, 100, 150, 200, 300, 400, 500, 1000] # The chunksizes that are going to be examined in the experiment
...
for e in exp_nums:
    ...
    for i in range(n_iter):
        ...
        for b in range(num_batches):                                            # this range is related to the number of splits
            ...
            numpy.savetxt("elec2/results/SGDResult" + str(n) + ".csv",  results, fmt='%.5f', delimiter=';', newline='\n') # export results file
            print(clf_score)
        ...
        numpy.savetxt("elec2/times/SGDTT" + str(n) + ".csv", times, fmt='%.5f')    # export file

    timesmean[0] = times.mean()
    numpy.savetxt("elec2/times/SGDTMEAN" + str(n) + ".csv", times, fmt='%.5f')  # export file
    ...
    numpy.savetxt("elec2/results/SGDAVG" + str(n) + ".csv", rmean, fmt='%.5f')
    numpy.savetxt("elec2/results/SGDSTD" + str(n) + ".csv", rstd, fmt='%.5f')
    ...
    numpy.savetxt("elec2/means/CONDRIFTRATE.csv", concept_drift_rate, fmt='%.5f')
...
numpy.savetxt("elec2/means/SGDAVGALLEXP.csv", rmeans, fmt='%.5f') # Averages
numpy.savetxt("elec2/means/SGDSTDALLEXP.csv", rstds, fmt='%.5f') # Standard Deviation
numpy.savetxt("elec2/means/SGDTIMESALLEXP.csv", timesmeans, fmt='%.5f') # Times
numpy.savetxt("elec2/means/SGDCDRALLEXP.csv", CDRmeans, fmt='%.5f') # Concept drift rate
numpy.savetxt("elec2/means/SGDPRECISION.csv", pre_means, fmt='%.5f') # Averages
numpy.savetxt("elec2/means/SGDALL.csv", rec_means, fmt='%.5f') # Averages
numpy.savetxt("elec2/means/SGDF1.csv", f1_means, fmt='%.5f') # Averages
